          ...
          <div className="flex items-center gap-10">
            ...
            <div className="relative hidden sm:block" ref={menuRef}>
              ...
              <div className="absolute right-0 mt-2 w-52 bg-white border border-border rounded-xl shadow-lg py-1 z-50">
                ...
                <div key={i} className="border-t border-border my-1" /> :
                item.label === 'Profile' ?
                <Link key={item.label} to="/profile" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                item.label === 'My Ads' ?
                <Link key={item.label} to="/my-ads" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                item.label === 'Messages' ?
                <Link key={item.label} to="/messages" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                item.label === 'Saved Searches' ?
                <Link key={item.label} to="/saved-searches" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                item.label === 'Browsing History' ?
                <Link key={item.label} to="/browsing-history" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                item.label === 'History Checks' ?
                <Link key={item.label} to="/history-checks" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                item.label === 'Payment History' ?
                <Link key={item.label} to="/payment-history" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                item.label === 'Help' ?
                <Link key={item.label} to="/help" onClick={() => setShowUserMenu(false)} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </Link> :
                <button key={item.label} onClick={() => { setShowUserMenu(false); if (item.action === 'logout') base44.auth.logout(window.location.origin + '/'); }} className="flex items-center gap-2.5 w-full px-4 py-2 text-sm text-foreground hover:bg-secondary transition-colors">
                          <item.icon className="w-4 h-4 text-muted-foreground" />{item.label}
                        </button>
                )}
                  </div>
      ...
      <div className="lg:hidden bg-primary border-t border-white/10 pb-4">
        ...
        <button key={link.label} className="w-full text-left text-white/90 hover:text-white hover:bg-white/10 px-6 py-3 text-sm font-medium flex items-center justify-between transition-colors">
          ...
          <div className="flex gap-6 px-6 pt-3 border-t border-white/10 mt-2">
            <button className="flex items-center gap-2 text-white/80 text-sm"><Search className="w-4 h-4" /> Search</button>
            <button className="flex items-center gap-2 text-white/80 text-sm font-semibold">Place Ad</button>
          </div>
        </div>
      }
    </nav>);
